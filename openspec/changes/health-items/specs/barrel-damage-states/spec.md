...
All hit-state tint values and count thresholds SHALL be defined as named constants in `EnemyConfig.ts` or a dedicated `PropConfig.ts`. No magic pixel values may appear inline in `DestructibleProp`.
