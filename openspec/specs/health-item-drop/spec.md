# health-item-drop Specification

## Purpose
Defines the health item drop behaviour when a barrel is destroyed: count selection, scatter positioning, and sushi sprite variant randomisation.
...
- **THEN** the item type is `'health'` and the `ItemPickup` created in `StageManager` uses `ASSET_KEY_PROP_SUSHI_1` or `ASSET_KEY_PROP_SUSHI_2`
...
- **THEN** `ITEM_DROP_SCATTER_RADIUS`, `ITEM_DROP_MIN_SPACING`, `ITEM_DROP_COUNT_MIN`, `ITEM_DROP_COUNT_MAX` (or equivalent names) are present as exports in `GameConfig.ts`
